...
class UserManager(BaseUserManager):
    ...
    def get_by_ci(self, ci, request=None, site=None):
        ...
        if request and apps.is_installed("django.contrib.sites"):
            return self.get(
                **{"profile__ci_hash": ci_hash, "site": get_current_site(request)}
            )
        elif site:
...
class Profile(models.Model):
    MOBILE_CARRIER = Choices(
        ...
        (
            "알뜰폰",
            [("04", "r_skt", "SKT"), ("05", "r_kt", "KT"), ("06", "r_lg", "LG U+")],
        ),
    )
    ...
    ci_hash = models.CharField(
        max_length=64, blank=True, null=True, help_text="고유 식별번호 Hash값", editable=False
    )
...
class ExpiringToken(Token):
    ...
    user = models.OneToOneField(
        User, related_name="auth_token", on_delete=models.CASCADE, verbose_name="User"
    )
