    ...
    def as_schema_property(self):
        if self.field_type == 'array':
            ...
            return {
                ...
                'schema': {
                    'properties': {k: {
                        'example': _get_field_type(v),
                        'type': _get_field_type(v),
                        'description': v.description
                    } for k, v in self.field.schema.properties.items()}
                }
            }
        ...
        return {
            'description': self.field_description,
            'type': self.field_type,
            'example': str(self.field.example) if self.field.example else self.field_type
        }
...
def _generate_openapi_object(document):
    ...
    swagger['basePath'] = urlparse.urljoin(document.url, document.version)
...
def _get_operation(operation_id, link, tags):
    ...
    if summary and re.match('^\[.+\]$', summary):
        summary = summary[1:-1]
...
def _get_responses(link):
    ...
    template.update({'description': responses.get(status_code, '')})

    res = {status_code: template}

    for error_status_code, schema in link.error_status_codes.items():
        if not schema.get('description', None):
            schema['description'] = responses.get(error_status_code, '')
