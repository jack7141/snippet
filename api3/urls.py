    ...
    for version, patterns in version_map_dict.items():
        ...
        sv = get_schema_view(
            openapi.Info(
                ...
                description=f"Account {version} Api List",
        ...
        urlpatterns = [url(r'^' + version + '/swagger/(?P<format>\.json|\.yaml)$', sv.without_ui(cache_timeout=0)),
                       url(r'^' + version + '/swagger/$', sv.with_ui('swagger', cache_timeout=0)),
                       url(r'^' + version + '/redoc/$', sv.with_ui('redoc', cache_timeout=0))] + urlpatterns
