...
class Event(Timestampable, models.Model):
    ...
    status = models.IntegerField(choices=STATUS, default=STATUS.on_hold, help_text=gen_choice_desc('주문상태', STATUS))
    mode = StatusField(choices_name='MODES', null=False, blank=False, default=MODES.new_order,
                       help_text=gen_choice_desc('주문 종류', MODES))
    portfolio_id = models.BigIntegerField(null=True, blank=True, default=None, help_text="주문 요청 포트폴리오 ID")
...
class OrderSetting(models.Model):
    EMPHASIS_TYPES = Choices(
        ('min_deposit', '예수금 최소화(O(n))'),
        ('strict_ratio', '비중 최적화(O(n))'),
        ('optimized_deposit', '예수금 최적화(O(2^n))'),
    )

    name = models.CharField(unique=True, max_length=128, null=False, blank=False, default="default", help_text='설정 별칭')
    strategies = JSONField(null=False, blank=False, default=get_default_strategies, help_text="주문 전략")
    min_base = models.IntegerField(default=0, help_text="최소 보유 원금")
    emphasis = models.CharField(max_length=100, default=EMPHASIS_TYPES.optimized_deposit, choices=EMPHASIS_TYPES,
...
class OrderDetail(Timestampable, models.Model):
    ...
    type = models.CharField(null=False, max_length=1, help_text=gen_choice_desc("매매 구분", ORDER_TYPE),
                            choices=ORDER_TYPE)
    ...
    shares = models.DecimalField(null=False, help_text="좌수", decimal_places=6, max_digits=30)
    result = models.CharField(null=False, max_length=1, help_text=gen_choice_desc("체결 구분", RESULT), choices=RESULT)
